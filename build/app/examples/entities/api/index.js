...
export class Entity extends AppContext {
    async get(payload) {
        new Validator(payload).setKey("id").isInt().isMin(1).setKey("keywords", true).isString().trimString().isMinLength(1).toLowerCase().setKey("myText", true).isString().trimString().isMinLength(3).isMaxLength(5).isTextPattern(myReGexp).setKey("myArrayUUID", true, null).isNull().forArray($0).setKey("myTuple").isMinLength(2).isArray().setKey("myEmail", true).isEmail().setKey("myPhone", true).isString().toDigitsString().isMinLength(3).setKey("myInt", true, 45).isInt().isMin(1).isMax(99).setKey("myFloat", true, 17.3).isNull().isNumber().isMin(10.5).isMax(20.01).setKey("myNumberArray", true).forArray($1).setKey("myUnion", true).inArray($2).setKey("myKeyOf", true).inArray($3).setKey("myEnum", true, Dir.Left).inArray($4).setKey("myObject", true).isObject().setKey("myObjectRecord", true).isObject().setKey("myStructObject", true).forObject($5).validate();
